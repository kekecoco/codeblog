...
2. 用户态和内核态的区别:
    ...
    * 由于需要限制不同的程序之间的访问能力,防止他们获取别的程序的内存数据,或者外围设备的数据并发送到网络,故CPU划分出两个权限等级: 用户态和内核态.
3. MySQL联合索引:
    * 当创建了(a,b,c)联合索引时,相当于创建了(a)单列索引.(a,b)联合索引以及(a,b,c)联合索引.
    * 想要索引生效的话,只能使用a和a,b和a,b,c三种组合.
    * 避免在where条件中使用or来连接条件,因为如果两个字段中有一个没有索引的话,引擎会放弃索引而产生全表扫描.
4. 聚簇索引:
    * 聚簇索引并不是一种单独的索引类型,而是一种数据处理方式
    * 术语聚簇表示数据行和相邻的键值紧凑的存储在一起.
    * 聚簇索引和二级索引: 叶子节点不会保存引用的行的物理位置,而是保存行的主键值.
5. 并发和并行:
    * 并发指的是程序的结构,并行指的是程序运行时的状态.
    * 并行:
        * 并行就是同时执行的意思.判断程序是否处于并行的状态,就看同一时间是否有超过一个工作单位在运行就好.
        * 要达到并行状态,最简单的就是利用多进程和多线程.
    * 并行指物理上同时执行,并发指能够让多个任务在逻辑上交织执行的程序设计.
6. 消息队列丢失数据的解决:
    * 丢失数据的场景:
        * 生产者丢失了数据.生产者将数据发送至消息队列的时候,可能在传输过程中因为网络等问题而将数据丢失.
        * 消息队列自己丢失了数据.消息队列没有开启或不支持持久化.
        * 消费端丢失了数据.消费者因为自己处理失败或宕机丢失了数据.
    * 解决办法:
        * 生产者在发送数据之前开启事务,然后发送消息,如果发送失败就回滚,成功则提交事务.
        * 使用confirm机制,发送消息后,如果消息队列接收成功,则异步通知发送成功.
        * 消息队列开启持久化机制.
        * 消费者在获取消息后需要为每条数据生成副本,一旦处理失败或异常宕机可以进行恢复.
